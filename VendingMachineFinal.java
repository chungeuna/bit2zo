...
class Payment {			// 제품 결제 클래스
	...
	void payMoney (Product s) {			// 결제 및 잔액 확인 기능
		if (s.productStockCount <= 0) {			// 품절상황 알림
			...
			System.out.println();
			System.out.println("< " + s.productName+"을 선택했습니다.  가격은 " + s.productPrice +"원 입니다 >");
	...
	void returnUnitMoney () {		// 잔액 권종별 반환 및 입금액 초기화 기능
		System.out.println("< 잔액 "+ insertMoney +"원을 반환합니다. 사용해주셔서 감사합니다! >");
		int unit = 10000;
		int num;
		int sw = 0;
		do {
		...
		}while((insertMoney%100==0) ? unit>=100 : unit>=0);		// 100원 단위 존재 여부에 따른 선택적 출력
		System.out.println();
		System.out.println();	
		insertMoney = 0;
	}
...
public class VendingMachineFinal {			// 자판기 메뉴 선택 클래스
	...
	void customerSelect (Product[] s) {			// 사용자 선택화면 구현
		...
		do {
			...
			switch (sel) {
			case 0: pay.returnUnitMoney();		// 잔액 반환 및 초기화 기능 호출
			...
			case 5:	pay.payMoney(s[4]);
				break;
			case 6: pay.insertExtraMoneyMethod();			// 추가 입금 기능 호출
				break;
			case 98909:	machineStart(s);		// 98909 입력 시 관리자 선택화면 호출
			}
		} while (true); 
	}
